...
namespace FKLib
{
    public static class EditorTools
    {
        ...
        private static Regex sArrayElementRegex = new Regex(@"\GArray\.data\[(\d+)\]", RegexOptions.Compiled);
        private static Dictionary<Type, ICustomDrawer>              _drawers;
        private static Dictionary<Type, DrawerKeySet>               _drawerTypeForType;
        private static Dictionary<Type, MonoScript>                 _typeMonoScriptLookup;
        private static Dictionary<Type, bool>                       _customPropertyDrawerLookup;

        static EditorTools()
        {
            _typeMonoScriptLookup = new Dictionary<Type, MonoScript>();
            _drawers = new Dictionary<Type, ICustomDrawer>();
            _customPropertyDrawerLookup = new Dictionary<Type, bool>();
        }

        public static string SearchField(string search, bool focus = false, 
        ...
        // Search field gui.
        public static string[] SearchField(string search, string filter, List<string> filters, 
            params GUILayoutOption[] options)
        {
        ...
        {
            if (GUILayout.Button(content, EditorToolsStyles.LeftTextButton, options))
            {
                return true;
            }
            return false;
        ...
        public static bool RightArrowButton(GUIContent content, params GUILayoutOption[] options)
        {
            ...
            GUI.Label(rect, EditorToolsStyles.RightArrow);
            return result;
        }
        ...
        {
            bool result = false;
            if (GUILayout.Button(content, EditorToolsStyles.LeftTextToolbarButton, options))
            {
                result = true;
            }
            Rect rect = GUILayoutUtility.GetLastRect();
        ...
        {
            bool result = false;
            if (GUI.Button(position, content, EditorToolsStyles.LeftTextButton))
            {
                result = true;
            }
            position.x += position.width - 20f;
            GUI.Label(position, EditorToolsStyles.RightArrow);
            return result;
        }

        public static void Seperator()
        {
            GUILayout.Label(GUIContent.none, EditorToolsStyles.Seperator);
        }
        ...
        public static bool Titlebar(string hash, GUIContent content, object target, GenericMenu menu)
        {
            ...
            Rect position = GUILayoutUtility.GetRect(GUIContent.none, EditorToolsStyles.InspectorTitle, GUILayout.ExpandWidth(true));
            if (Event.current.type == EventType.Repaint)
            {
                Color color = GUI.color;
                GUI.color = position.Contains(Event.current.mousePosition) ? color * 1.3f : color * 1.1f;
                EditorToolsStyles.InspectorBigTitle.Draw(new Rect(position.x, position.y, position.width + 4f, position.height), GUIContent.none, controlID, false);
                GUI.color = color;
            }

            Rect rect = new Rect(position.x + (float)EditorToolsStyles.InspectorTitle.padding.left, position.y + (float)EditorToolsStyles.InspectorTitle.padding.top, 16f, 16f);
            Rect rect1 = new Rect(position.xMax - (float)EditorToolsStyles.InspectorTitle.padding.right - 2f - 16f, rect.y, 16f, 16f);
            Rect rect4 = rect1;
            rect4.x = rect4.x - 18f;
            ...
            if (menu != null && GUI.Button(rect1, EditorGUIUtility.FindTexture("d__Menu"), EditorToolsStyles.InspectorTitleText))
            {
                menu.ShowAsContext();
            }
            ...
            if (eventType != EventType.MouseDown)
            {
                ...
                {
                    EditorToolsStyles.InspectorTitle.Draw(position, GUIContent.none, controlID, isFolded);
                    EditorToolsStyles.InspectorTitleText.Draw(rect2, content, controlID, isFolded);
                }
            }
        ...
        public static MonoScript FindMonoScript(Type type)
        {
            ...
            if (!EditorTools._typeMonoScriptLookup.TryGetValue(type, out monoScript))
            {
                ...
                foreach (string assetPath in assetPaths)
                {
                    if (assetPath.EndsWith(".cs"))
                    {
                        ...
                        {
                            EditorTools._typeMonoScriptLookup.Add(type, script);
                            return script;
                        }
                    }
                }
        ...
        private static bool NextPropertyPath(string propertyPath, ref int index, out PropertyPath component)
        {
            ...
            var arrayElementMatch = sArrayElementRegex.Match(propertyPath, index);
        ...
        {
            if (EditorTools._customPropertyDrawerLookup.ContainsKey(type))
            {
                return EditorTools._customPropertyDrawerLookup[type];
            }
            ...
            foreach (Type typesDerivedFrom in TypeCache.GetTypesDerivedFrom<GUIDrawer>())
            {
                ...
                for (int i = 0; i < (int)customAttributes.Length; i++)
                {
                    ...
                    if (type == type1)
                    {
                        EditorTools._customPropertyDrawerLookup.Add(type, true);
                        return true;
                    }
                }
            }
            EditorTools._customPropertyDrawerLookup.Add(type, false);
            return false;
        }
        ...
        public static object DrawField(Rect rect, GUIContent label, object obj, object value, FieldInfo field, params GUILayoutOption[] options)
        {
            ...
            ICustomDrawer customDrawer = GetCustomDrawer(type);
        ...
        public static object DrawField(GUIContent label, object obj, object value, FieldInfo field, params GUILayoutOption[] options)
        {
            ...
            if (customDrawer != null)
            {
                ...
                customDrawer.Value = value;
                customDrawer.OnGUI(label);
                if (customDrawer.IsDirty)
                {
                    GUI.changed = true;
                    customDrawer.IsDirty = false;
                }
                return value;
            }
        ...
        private static ICustomDrawer GetCustomDrawer(Type type)
        {
            Type drawerType = GetDrawerTypeForType(type);
            if (drawerType != null)
            {
                ICustomDrawer drawer;
                if (!EditorTools._drawers.TryGetValue(drawerType, out drawer))
                {
                    drawer = (ICustomDrawer)System.Activator.CreateInstance(drawerType);
                    EditorTools._drawers.Add(drawerType, drawer);
                }
                return drawer;
            }
            return null;
        ...
        private static Type GetDrawerTypeForType(Type type)
        {
            ...
            Type type1;
            if (EditorTools._drawerTypeForType == null)
            {
                EditorTools.BuildDrawerTypeForTypeDictionary();
            }
            EditorTools._drawerTypeForType.TryGetValue(type, out drawerKeySet);
            if (drawerKeySet.Drawer == null)
            {
                if (type.IsGenericType)
                {
                    EditorTools._drawerTypeForType.TryGetValue(type.GetGenericTypeDefinition(), out drawerKeySet);
                }
        ...
        {
            EditorTools._drawerTypeForType = new Dictionary<Type, EditorTools.DrawerKeySet>();
            foreach (Type typesDerivedFrom in TypeCache.GetTypesDerivedFrom<ICustomDrawer>())
            {
                ...
                for (int i = 0; i < customAttributes.Length; i++)
                {
                    ...
                    Dictionary<Type, EditorTools.DrawerKeySet> sDrawerTypeForType = EditorTools._drawerTypeForType;
                    ...
                    if (customDrawer.IsUseForChildren)
                    {
                        foreach (Type type in TypeCache.GetTypesDerivedFrom(customDrawer.Type))
                        {
                            if ((!EditorTools._drawerTypeForType.ContainsKey(type) ? true : !customDrawer.Type.IsAssignableFrom(EditorTools._drawerTypeForType[type].Type)))
                            {
                                Dictionary<Type, EditorTools.DrawerKeySet> types = EditorTools._drawerTypeForType;
