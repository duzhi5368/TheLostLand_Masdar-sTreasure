...
namespace FKLib
{
    ...
    public class StatPickerAttributeDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            EditorGUI.BeginProperty(position, label, property);
            IStat current = property.GetValue() as IStat;
            position = EditorGUI.PrefixLabel(position, label);
            DoSelection(position, property, label, current);
            EditorGUI.EndProperty();
        }

        protected virtual void DoSelection(Rect buttonRect, SerializedProperty property, GUIContent label, IStat current)
        {
        ...
        protected virtual List<IStat> GetItems(StatDatabase database)
        {
            ...
            return database.Stats.Where(x => type.IsAssignableFrom(x.GetType())).ToList();
        }
