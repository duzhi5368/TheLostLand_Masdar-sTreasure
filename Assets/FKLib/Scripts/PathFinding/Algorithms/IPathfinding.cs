...
namespace FKLib
{
    public abstract class IPathfinding
    {
        ...
        public abstract Dictionary<T, IList<T>> FindAllPaths<T>(Dictionary<T, Dictionary<T, float>> edges, T originNode) where T : IGraphNode;
        // 从图中检索指定节点的相邻节点
        // Params: edges - 图的边结构
        // Params: node - 需要进行检索的节点
        // Return: 相邻节点；若无邻接节点，则返回null
        protected IEnumerable<T> GetNeigbors<T>(Dictionary<T, Dictionary<T, float>> edges, T node) where T : IGraphNode
        {
