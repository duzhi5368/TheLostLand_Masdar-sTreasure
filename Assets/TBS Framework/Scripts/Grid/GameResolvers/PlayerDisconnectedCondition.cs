        ...
        public override GameResult CheckCondition(CellGrid cellGrid)
        {
            if(_networkPlayers.Count == 1)
            {
                return new GameResult(isFinished: true, 
                    FindObjectsByType<Player>(FindObjectsSortMode.None).ToList().Where(p => p is HumanPlayer).Select(p => p.PlayerNumber).ToList(),
                    FindObjectsByType<Player>(FindObjectsSortMode.None).ToList().Where(p => p is RemotePlayer).Select(p => p.PlayerNumber).ToList());
            }
            else
            {
